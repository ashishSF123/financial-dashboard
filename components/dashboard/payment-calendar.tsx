...
export function PaymentCalendar({ data, selectedMonth }: Props) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="hidden md:flex items-center gap-4">
          {Object.entries(categoryConfig).map(([cat, c]) => (
            <div key={cat} className="flex items-center gap-1.5">
              ...
              <span className="text-[0.65rem] text-slate-500 font-medium">{c.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-[#12131a] border border-white/[0.06] rounded-xl px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[0.6rem] uppercase tracking-[0.08em] font-semibold text-slate-500">Total Due</p>
              <p className="text-[1.1rem] font-bold text-white tracking-tight mt-0.5">{formatINR(totalDueThisMonth)}</p>
            </div>
      ...
      <div className="bg-[#12131a] border border-white/[0.06] rounded-2xl p-4 max-w-md mx-auto">
        {/* Day headers */}
        <div className="grid grid-cols-7 mb-1">
          {dayNames.map((d) => (
            <div key={d} className="text-center text-[0.55rem] font-semibold text-slate-500 tracking-[0.08em] py-1">
              {d}
            </div>
          ))}
        </div>
            ...
            return (
              <button
                ...
                className={`relative h-9 rounded-lg flex flex-col items-center justify-center transition-all duration-150 ${
                  isToday
                    ? "bg-indigo-500/20 ring-1.5 ring-indigo-500/50 text-white font-bold"
                    : isSelected
                    ? "bg-white/[0.08] ring-1 ring-white/20 text-white"
                    : hasPayments
                    ? "bg-white/[0.025] hover:bg-white/[0.06] text-slate-300 cursor-pointer"
                    : "text-slate-600 hover:text-slate-500 hover:bg-white/[0.015]"
                }`}
              >
        ...
        {isCurrentMonth && (
          <div className="flex items-center gap-2 mt-3 pt-2.5 border-t border-white/[0.04]">
            ...
            <span className="text-[0.63rem] text-slate-500">Today is {monthName} {today}</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#12131a] border border-white/[0.06] rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-white/[0.03]">
            ...
            return (
              <div key={p.id} className="flex items-center justify-between px-5 py-3 hover:bg-white/[0.015] transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[0.8rem] text-slate-200 font-medium">{p.name}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[0.65rem] text-slate-500">Due {p.dueDay}th</span>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-[0.85rem] font-semibold text-white tabular-nums">{formatINR(p.amount)}</span>
          ...
          {filteredPayments.length === 0 && (
            <div className="text-center py-8">
              <p className="text-slate-500 text-[0.8rem]">No payments scheduled for this day</p>
            </div>
          )}
        </div>
        ...
        {filteredPayments.length > 0 && (
          <div className="px-5 py-3 border-t border-white/[0.04] bg-white/[0.01] flex items-center justify-between">
            <span className="text-[0.68rem] text-slate-500">{filteredPayments.length} payment{filteredPayments.length > 1 ? "s" : ""}</span>
            <span className="text-[0.78rem] font-semibold text-white tabular-nums">
