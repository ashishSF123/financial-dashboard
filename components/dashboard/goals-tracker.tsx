    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[0.78rem] text-slate-500 leading-relaxed mt-1">Track progress toward your savings targets</p>
        </div>
        <button
      ...
      <div className="bg-gradient-to-br from-indigo-500/[0.06] to-purple-500/[0.03] border border-indigo-500/20 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-[0.68rem] text-slate-500 mt-0.5">{goals.length} active goals · {formatINR(totalSaved)} saved of {formatINR(totalTargeted)}</p>
          </div>
        ...
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-4 space-y-3">
          ...
          <form onSubmit={(e) => {
            ...
            <div className="grid grid-cols-3 gap-3">
              <input name="name" placeholder="Goal name" className="text-xs bg-white/[0.05] border border-white/[0.1] rounded-lg px-3 py-2 text-white outline-none placeholder-slate-600 focus:border-indigo-500/50" />
              <input name="target" type="number" placeholder="Target ₹" className="text-xs bg-white/[0.05] border border-white/[0.1] rounded-lg px-3 py-2 text-white outline-none placeholder-slate-600 focus:border-indigo-500/50" />
              <input name="deadline" type="month" className="text-xs bg-white/[0.05] border border-white/[0.1] rounded-lg px-3 py-2 text-white outline-none focus:border-indigo-500/50" />
            </div>
            <div className="flex gap-2 mt-3">
              ...
              <button type="button" onClick={() => setShowAddForm(false)} className="px-3 py-1.5 text-xs text-slate-500 border border-white/[0.06] rounded-lg">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {goals.map((goal) => {
          ...
          return (
            <div key={goal.id} className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-5 group relative">
              {/* Delete button */}
              <button
                onClick={() => setGoals((prev) => prev.filter((g) => g.id !== goal.id))}
                className="absolute top-3 right-3 text-slate-600 hover:text-rose-400 opacity-0 group-hover:opacity-100 transition-all text-xs"
              >
                ✕
              </button>
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex justify-between text-[10px] text-slate-500">
                    <span>Saved: <span className="text-slate-300">{formatINR(goal.current)}</span></span>
                    <span>Target: <span className="text-slate-300">{formatINR(goal.target)}</span></span>
                  </div>
              ...
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg p-2.5 flex items-center justify-between">
                <span className="text-[0.68rem] text-slate-500">Monthly contribution needed</span>
      ...
      <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-6">
        <h3 className="text-[0.88rem] font-semibold text-white tracking-[-0.01em] mb-3">Suggested Monthly Allocation</h3>
        <p className="text-[0.68rem] text-slate-500 mb-4">Based on your surplus of {formatINR(availableSurplus)} and goal priorities</p>
        <div className="space-y-2">
          {goals.sort((a, b) => {
            ...
            return (
              <div key={goal.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-white/[0.02]">
                <span>{goal.icon}</span>
                <span className="text-xs text-slate-300 flex-1">{goal.name}</span>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-slate-300 w-16 text-right">{formatINR(allocated)}</span>
                </div>
              </div>
            );
          })}
