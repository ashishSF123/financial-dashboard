...
export function NetWorthTimeline({ data }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-[1.15rem] font-semibold tracking-[-0.02em] text-[var(--text-heading)]">Net Worth Timeline</h2>
        <p className="text-[0.78rem] text-[var(--text-muted)] mt-0.5">Track your total wealth trajectory over time</p>
      </div>
            ...
            {history.slice(-12).map((h) => {
              ...
              return (
                <div key={h.month} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  />
                  <span className="text-[0.5rem] text-[var(--text-muted)]">{h.month.split("-")[1]}</span>
                </div>
              );
            })}
          </div>
          {monthlyGrowth !== 0 && (
            <p className="text-[0.68rem] text-[var(--text-muted)] mt-3">
      ...
      {monthlyIncome > 0 && (
        <div className="bg-[var(--bg-secondary)] border border-[var(--border-card)] rounded-2xl p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)]">
              <p className="text-[0.6rem] text-[var(--text-muted)] uppercase tracking-wider">Now</p>
              <p className="text-[0.9rem] font-bold text-[var(--text-heading)] mt-1">{formatINR(latest?.netWorth || 0)}</p>
            </div>
            <div className="text-center p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)]">
              <p className="text-[0.6rem] text-[var(--text-muted)] uppercase tracking-wider">6 Months</p>
              <p className="text-[0.9rem] font-bold text-emerald-400 mt-1">{formatINR(projected6mo)}</p>
            </div>
            <div className="text-center p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-subtle)]">
              <p className="text-[0.6rem] text-[var(--text-muted)] uppercase tracking-wider">1 Year</p>
