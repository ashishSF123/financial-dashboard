  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[0.78rem] text-[var(--text-muted)] leading-relaxed mt-1">Track who owes whom with real-time interest accrual</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-rose-500/[0.05] border border-rose-500/20 rounded-xl p-4">
          ...
          <p className="text-[0.68rem] text-[var(--text-muted)] mt-1">{analysis.borrowedPeople.length} creditors</p>
        </div>
        <div className="bg-emerald-500/[0.05] border border-emerald-500/20 rounded-xl p-4">
          ...
          <p className="text-[0.68rem] text-[var(--text-muted)] mt-1">{analysis.lendedByPerson.size} debtors</p>
        </div>
        <div className={`border rounded-xl p-4 ${analysis.netPosition >= 0 ? "bg-emerald-500/[0.05] border-emerald-500/20" : "bg-rose-500/[0.05] border-rose-500/20"}`}>
          <p className="text-[0.65rem] uppercase tracking-[0.08em] font-medium text-[var(--text-secondary)] mb-1">Net Position</p>
          ...
          <p className="text-[0.68rem] text-[var(--text-muted)] mt-1">{analysis.netPosition >= 0 ? "Net positive" : "Net negative"}</p>
        </div>
      </div>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-[0.88rem] font-semibold text-[var(--text-heading)] tracking-[-0.01em]">People You Owe</h3>
        </div>
        <div className="space-y-3">
          {analysis.borrowedPeople.map((person) => (
            <div key={person.id} className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[var(--text-primary)]">{person.personName}</p>
                    <p className="text-[0.68rem] text-[var(--text-muted)]">{person.location} · Since {person.date}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-base font-bold text-rose-400">{formatINR(person.totalOwed)}</p>
                  <p className="text-[0.68rem] text-[var(--text-muted)]">Total owed now</p>
                </div>
              </div>
              <div className="grid grid-cols-4 gap-2 bg-[var(--bg-card)] rounded-lg p-2">
                <div className="text-center">
                  <p className="text-[9px] text-[var(--text-muted)] uppercase">Principal</p>
                  <p className="text-xs font-medium text-[var(--text-secondary)]">{formatINR(person.amount)}</p>
                </div>
                <div className="text-center">
                  <p className="text-[9px] text-[var(--text-muted)] uppercase">ROI</p>
                  <p className="text-xs font-medium text-[var(--text-secondary)]">{person.roi}%/mo</p>
                </div>
                <div className="text-center">
                  <p className="text-[9px] text-[var(--text-muted)] uppercase">Months</p>
                  <p className="text-xs font-medium text-[var(--text-secondary)]">{person.months}</p>
                </div>
                <div className="text-center">
                  <p className="text-[9px] text-[var(--text-muted)] uppercase">Int. Accrued</p>
              ...
              <div className="mt-3">
                ...
                <div className="h-1.5 rounded-full bg-[var(--bg-card-hover)] overflow-hidden flex">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-card)] rounded-2xl p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-[0.88rem] font-semibold text-[var(--text-heading)] tracking-[-0.01em]">People Who Owe You</h3>
        </div>
        <div className="space-y-3">
          {Array.from(analysis.lendedByPerson.entries()).map(([personName, entries]) => {
            ...
            return (
              <div key={personName} className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-[var(--text-primary)]">{personName}</p>
                      <p className="text-[0.68rem] text-[var(--text-muted)]">{entries.length} transaction{entries.length > 1 ? "s" : ""} · {entries[0]?.location}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-base font-bold text-emerald-400">{formatINR(total)}</p>
                    <p className="text-[0.68rem] text-[var(--text-muted)]">Total receivable</p>
                  </div>
                </div>
                {/* Breakdown per transaction */}
                <div className="space-y-1.5">
                  {entries.map((entry) => (
                    <div key={entry.id} className="flex items-center justify-between text-xs bg-[var(--bg-card)] rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-[var(--text-muted)]">{entry.date}</span>
                        <span className="text-[var(--text-secondary)]">{formatINR(entry.amount)}</span>
                        <span className="text-[var(--text-muted)]">@</span>
                        <span className="text-[var(--text-secondary)]">{entry.roi}%/mo</span>
                      </div>
                      <div className="flex items-center gap-2">
      ...
      <div className="bg-gradient-to-br from-emerald-500/[0.05] to-cyan-500/[0.03] border border-emerald-500/20 rounded-2xl p-6">
        <h3 className="text-[0.88rem] font-semibold text-[var(--text-heading)] tracking-[-0.01em] mb-3">Monthly Interest Flow</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-[0.65rem] text-[var(--text-muted)] uppercase tracking-[0.08em] font-medium mb-0.5">Interest You Pay</p>
            <p className="text-lg font-bold text-rose-400">-{formatINR(data.borrowed.reduce((s, b) => s + b.monthlyInterest, 0))}/mo</p>
          </div>
          <div>
            <p className="text-[0.65rem] text-[var(--text-muted)] uppercase tracking-[0.08em] font-medium mb-0.5">Interest You Earn</p>
            <p className="text-lg font-bold text-emerald-400">+{formatINR(data.lended.reduce((s, l) => s + l.monthlyInterest, 0))}/mo</p>
          </div>
          <div>
            <p className="text-[0.65rem] text-[var(--text-muted)] uppercase tracking-[0.08em] font-medium mb-0.5">Net Interest</p>
            {(() => {
